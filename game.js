...
function press() { if (state === 'ready') startGame(); else if (state === 'playing') { velocity = -CONFIG.flapStrength; tapCount += 1; if (tapCount % 3 === 0) hatBirds.push({ hat: hatBirds.length % 4, y: y - 42 - (hatBirds.length % 2) * 24 }); play('flap'); } else if (state === 'gameover' && secondsSinceCrash >= 0.4) startGame(); }
